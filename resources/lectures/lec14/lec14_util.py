...
def train_and_plot(train_sample, test_sample, degs):
    ...
    rmses = [np.sqrt(np.mean((pred_funcs[i].predict(test_sample[['x']]) - test_sample['y']) ** 2)) for i in range(len(pred_funcs))]
    titles = [f'Degree {degs[i]}, RMSE: {np.round(rmses[i], 3)}' for i in range(len(pred_funcs))]

    fig = make_subplots(rows=1, cols=len(pred_funcs), subplot_titles=titles)

    for i in range(len(pred_funcs)):
        fig.add_trace(
            go.Scatter(x=test_sample['x'], y=test_sample['y'], name='Actual Data', 
                       mode='markers', marker={'color': '#4c72b0'}),
            row=1, col=i+1
        )

        fig.add_trace(
            go.Scatter(x=test_sample['x'], y=pred_funcs[i].predict(test_sample[['x']]), name='Predictions', 
...
def plot_multiple_models(sample_1, sample_2, degs):
    ...
    fig = make_subplots(rows=1, cols=len(degs), subplot_titles=[f'Degree {deg}' for deg in degs])

    for i in range(len(degs)):
        fig.add_trace(
            go.Scatter(x=sample_1['x'], y=sample_1_funcs[i].predict(sample_1[['x']]),
                       line={'color': 'orange', 'width': 4}),
            row=1, col=i+1
        )

        fig.add_trace(
            go.Scatter(x=sample_2['x'], y=sample_2_funcs[i].predict(sample_2[['x']]),
